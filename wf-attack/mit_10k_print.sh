#!/bin/bash

'''
get print for all 10k mit results
./wf-attack-vpn/wf-attack/mit_10k_print.sh
'''

python wf-attack-vpn/wf-attack/mit_10k_print.py --app youtube -r mit_10k_print -i mit_10k
python wf-attack-vpn/wf-attack/mit_10k_print.py --app netflix -r mit_10k_print -i mit_10k
python wf-attack-vpn/wf-attack/mit_10k_print.py --app vimeo   -r mit_10k_print -i mit_10k

python wf-attack-vpn/plot_graphs/plot_all.py -r wf_result/mit_10k_print/ -g fig/mit_10k/ -w 10 --ylim_lower 0